# v. 06 mar 2026 09:30 (CONTROL DE INTEGRIDAD INTERNO: 188 LÍNEAS)
...
def llamar_ia_en_cascada(prompt):
    ...
    # Para debug, mostrar modelos detectados
    st.sidebar.write("🛠️ Modelos disponibles:", modelos_disponibles)
    ...
    # Fallback seguro: probar gpt-3.5-turbo directamente aunque no aparezca en la lista
    try:
        response = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[{"role":"user","content":prompt}],
        ...
        st.session_state.active_model = "gpt-3.5-turbo (fallback)"
        return response.choices[0].message.content
    except Exception:
...
st.markdown('<div class="sub-version">v. 06 mar 2026 09:30</div>', unsafe_allow_html=True)

# (El resto del código de interfaz, pestañas, registro de paciente, calculadora,
# glow box, listado de medicamentos, validación y pestaña de informes
# permanece exactamente igual que tu versión anterior.)
