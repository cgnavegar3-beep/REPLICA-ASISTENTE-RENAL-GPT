...
# =================================================================
# FUNCIÓN DE LLAMADA A IA EN CASCADA CON VERIFICACIÓN DE MODELOS
# =================================================================
def listar_modelos_disponibles():
    """Devuelve la lista de modelos disponibles con la API key actual"""
    if not API_KEY:
        return []
    try:
...
def llamar_ia_en_cascada(prompt):
    ...
    modelos_preferidos = ["gpt-4o-mini", "gpt-4o"]
    disponibles = listar_modelos_disponibles()
    modelos_uso = [m for m in modelos_preferidos if m in disponibles]

    if not modelos_uso:
        return "⚠️ Error en la generación: ningún modelo disponible."

    for m in modelos_uso:
        try:
            client = OpenAI(api_key=API_KEY)
...
with tabs[0]:
    ...
    with c3:
        st.text_input("Fecha", value=datetime.now().strftime("%d/%m/%Y"), disabled=True, placeholder="Fecha")
    with c4:
        st.text_input("ID Registro", key="reg_id", disabled=True)
    with c5:
        st.write(""); st.button("🗑️", on_click=reset_registro, key="btn_reset_reg")
