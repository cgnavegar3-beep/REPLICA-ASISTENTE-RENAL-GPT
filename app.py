# v. 06 mar 2026 11:32 (CONTROL DE INTEGRIDAD INTERNO: 188 LÍNEAS)
...
def llamar_ia_en_cascada(prompt):
    """
    Función de llamada a IA con doble checking de modelos.
    Solo modifica el flujo de IA, respetando toda la interfaz y funcionalidades.
    """
    if not API_KEY:
...
def inject_styles():
    st.markdown("""
    <style>
    /* TODO: conservar todo tu CSS actual intacto */
...
st.markdown('<div class="sub-version">v. 06 mar 2026 11:32</div>', unsafe_allow_html=True)
...
with tabs[0]:
    ...
    with col_der:
        ...
        with l1:
            ...
            val_mdrd = st.number_input("MDRD-4", value=None, key="fgl_mdrd", label_visibility="collapsed", placeholder="MDRD-4")
            st.markdown('</div><div class="unit-label">mL/min/1,73m²</div>', unsafe_allow_html=True)
        with l2:
            st.markdown('<div class="fg-special-border">', unsafe_allow_html=True)
            val_ckd = st.number_input("CKD-EPI", value=None, key="fgl_ckd", label_visibility="collapsed", placeholder="CKD-EPI")
            st.markdown('</div><div class="unit-label">mL/min/1,73m²</div>', unsafe_allow_html=True)
